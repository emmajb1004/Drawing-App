...
function Toolbox() {
	...
	this.selectTool = function(toolName) {
		...
		this.getDraggableCircle = function() {
			//search through the tools for one that's name matches draggableShapesCircle and return name
			for (let i = 0; i < this.tools.length; i++) {
				if (this.tools[i].name == "draggableShapesCircle") {
					return this.tools[i];
				}
			}
	}

	this.getDraggableSquare = function() {
		//search through the tools for one that's name matches draggableShapesSquare and return name
		for (let i = 0; i < this.tools.length; i++) {
			if (this.tools[i].name == "draggableShapesSquare") {
				return this.tools[i];
			}
		}
}
